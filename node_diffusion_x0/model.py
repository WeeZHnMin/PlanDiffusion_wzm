...
class NodeDiffusionTransformer(nn.Module):
    ...
    def __init__(self, model_channels=384, num_layers=6, num_heads=6,
        ...
        # 类型预测头：输出 logits [B, N, 33]（0=padding，1-32=有效类型）
        self.type_head = nn.Sequential(
            nn.LayerNorm(model_channels),
            nn.Linear(model_channels, model_channels),
            nn.ReLU(),
            nn.Linear(model_channels, N_TYPES + 1),  # 33
        )
    ...
    def forward(self, x, timesteps, adj_matrix, node_mask,
        ...
        pred_x0      = self.coord_head(seq).permute(0, 2, 1)   # [B, 2, N]
        type_logits  = self.type_head(seq)                      # [B, N, 33]
        return pred_x0, type_logits
