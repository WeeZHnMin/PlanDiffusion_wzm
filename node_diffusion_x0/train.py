...
from .dataset import load_node_data, TypeDataset
from torch.utils.data import DataLoader


def load_type_data(npz_path, batch_size, shuffle=True):
    dataset = TypeDataset(npz_path)
...
def build_parser(defaults=None):
    ...
    parser.add_argument("--total_steps", type=int, default=defaults.get("total_steps", 500000))
...
def main(argv=None, defaults=None):
    ...
    data = load_type_data(args.data_path, args.batch_size, shuffle=True)

    model.train()
    running_loss = running_coord = running_type_loss = running_rmse = running_type_acc = 0.0
    ...
    for step in range(start_step, args.total_steps):
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
            loss, coord_loss, type_loss, coord_rmse, type_acc = \
                diffusion.training_losses(model, x, t, cond)
        ...
        if step % args.log_interval == 0 and step > 0:
            ...
            avg_type_loss = running_type_loss / n
            avg_rmse      = running_rmse      / n
            avg_type_acc  = running_type_acc  / n
            running_loss = running_coord = running_type_loss = running_rmse = running_type_acc = 0.0
            elapsed = time.perf_counter() - t0
            t0      = time.perf_counter()

            print(f"step {step:6d} | loss {avg_loss:.4f} | coord {avg_coord:.4f} | "
                  f"type {avg_type_loss:.4f} | rmse {avg_rmse:.2f} | "
                  f"type_acc {avg_type_acc:.3f} | {elapsed:.1f}s")
            log_file.write(json.dumps({
                'step': step,
                'loss': round(avg_loss, 4),
                'coord_loss': round(avg_coord, 4),
                'type_loss': round(avg_type_loss, 4),
                'coord_rmse': round(avg_rmse, 2),
                'type_acc': round(avg_type_acc, 4),
        ...
        if step > 0 and step % args.save_interval == 0:
            ...
            print(f"  saved -> {ckpt_path} (step {step})")
