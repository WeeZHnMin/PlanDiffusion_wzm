"""
构建 node_diffusion_room_tri（三流注意力版）训练用 NPZ。

相比 node_diffusion_room 额外保存：
  adj_matrix      : (N, 40, 40)         uint8,   二值邻接矩阵（adj_attn 用）
  room_membership : (N, 40, MAX_ROOMS)  float32，节点-环隶属矩阵（room_attn 用）
...
用法：
  ...
  python -m node_diffusion_room_tri.build_graph_npz \\
      --max_samples 5000 \\
      --output data/processed/node_diffusion_room_tri/graph_dataset_5k.npz
"""
...
def main():
    ...
    np.savez_compressed(
        ...
        adj_matrix       = adj_arr.astype(np.uint8),   # [total, N, N] uint8 省空间
    )
