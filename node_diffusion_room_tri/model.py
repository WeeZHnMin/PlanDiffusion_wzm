"""
NodeDiffusionTransformer — 三流自注意力 (Tri-Stream Self-Attention)
...
三路独立学习不同几何归纳偏置，共享 FFN 和 cross-attn。
...
class NodeDiffusionTransformer(nn.Module):
    ...
    def __init__(self, model_channels=384, num_layers=6, num_heads=6,
        ...
        print(f"NodeDiffusionTransformer(adj+room+global tri-stream): {trainable:,} trainable / {total:,} total")
    ...
    def forward(self, x, timesteps, node_mask,
        ...
        for layer in self.layers:
            seq = layer(seq, room_mask, text_feat, text_mask, pad_mask, adj_mask)
