...
class Dataset(Dataset):
    def __init__(
        ...
        self.mask = mask
        self.onehot=onehot
        # 以 CSV 为索引，按 CSV 中记录的文件名加载图片和 mask，
        # 这样可以通过 CSV 过滤样本（如剔除房间数 >10 的），无需删除文件。
        ...
        self.image_paths = [Path(f"{folder_image}") / row[0] for row in self.texts]
        self.mask_paths  = [Path(f"{folder_mask}")  / row[0] for row in self.texts]
        assert (
