"""
交叉注意力版本：文本全序列 token 通过交叉注意力条件化每一层节点特征
- x0-prediction：模型直接预测干净坐标
- 文本条件：BERT 全序列 [B, seq_len, 768] 作为 key/value，节点特征作为 query
- 每个 Transformer 层 = 自注意力 → 交叉注意力 → FFN
"""
...
print("Encoding prompts (full token sequence)...")
with torch.no_grad():
    # 统一 padding 到同一长度，方便批量处理
    inputs = tokenizer(
    ...
    out    = bert(**inputs)
    # text_encs: [N, seq_len, 768]  全序列 token 特征
    # text_mask: [N, seq_len]       1=有效 token，0=padding
    text_encs = out.last_hidden_state               # [N, seq_len, 768]
    text_mask = inputs["attention_mask"].float()    # [N, seq_len]

SEQ_LEN = text_encs.size(1)
print(f"Text token sequence: {text_encs.shape}")
...
class CrossAttnLayer(nn.Module):
    ...
    def forward(self, x, text_kv, self_attn_mask=None, node_key_pad=None, text_key_pad=None):
        ...
        x = x + self.self_attn(h, h, h,
                                ...
                                key_padding_mask=node_key_pad)[0]
        # 交叉注意力（节点 query → 文本 key/value）
        h = self.norm2(x)
        x = x + self.cross_attn(h, text_kv, text_kv,
                                  key_padding_mask=text_key_pad)[0]
        # FFN
...
class NodeDiffusionCrossAttn(nn.Module):
    def __init__(self, d_model=D_MODEL, n_heads=N_HEADS,
        ...
        # 文本 token 从 768 投影到 d_model，作为交叉注意力的 key/value
        self.text_kv_proj = nn.Linear(768, d_model)
        self.layers = nn.ModuleList([
            CrossAttnLayer(d_model, n_heads) for _ in range(n_layers)
        ])
        self.final_norm = nn.LayerNorm(d_model)
        self.out        = nn.Linear(d_model, 2)

    def forward(self, noisy_coords, t_idx, text_enc, text_pad_mask,
        ...
        # 文本 key/value：[B, seq_len, d_model]
        text_kv = self.text_kv_proj(text_enc)

        # 交叉注意力的 key_padding_mask：padding token 位置为 True（被忽略）
        text_key_pad = (text_pad_mask == 0)   # [B, seq_len]

        # 节点 padding mask（自注意力用）
        node_key_pad = ((node_mask == 0).float() * -1e9) if node_mask is not None else None

        # 邻接掩码（自注意力用）
        self_attn_mask = None
        if self.use_adj and adj is not None:
            real_row       = node_mask.unsqueeze(-1)
            self_attn_mask = ((1.0 - adj) * (-1e9) * real_row
        ...
        return self.out(self.final_norm(x))   # [B, N_MAX, 2]  pred_x0


# ── 训练 + 推理 + 可视化 ──────────────────────────────────────────────────────
def train_version(use_adj, label):
    print(f"\n{'='*60}")
    print(f"版本：{'有邻接掩码' if use_adj else '无邻接掩码'}  ({label})  [cross-attn x0-pred]")
    print(f"{'='*60}")

    model = NodeDiffusionCrossAttn(use_adj=use_adj).to(DEVICE)
    ...
    with torch.no_grad():
        ...
        for step in reversed(range(T)):
            ...
            if step == 0:
                ...
                x    = mean + posterior_var[step].sqrt() * torch.randn_like(x) * mask3
    ...
    fig.suptitle(f"Node Diffusion CrossAttn — {label}  整体MAE={mae_pixel:.2f}px", fontsize=11)
    fig.tight_layout()
    out_path = OUT_DIR / f"result_gcn_cross_attn_{label}.png"
    ...
    save_path = weights_dir / f"diffusion_gcn_cross_attn_{label}.pt"
