"""
NodeDataset for node_diffusion_rid：在原有 cond 字典里额外附带 room_ids。

npz 中必须含有 room_ids 字段（用 precompute_room_ids.py 提前生成）。
"""
...
class NodeDataset(Dataset):
    ...
    Each sample returns:
      ...
      cond : dict with
               ...
               node_mask     [40]      1=有效节点
               room_ids      [40]      房间编号（0=无房间/填充，1..K=房间）
    ...
    def __init__(self, npz_path):
        ...
        if 'room_ids' not in d:
            raise KeyError(
                f"npz '{npz_path}' 缺少 room_ids 字段。\n"
                "请先运行：\n"
                "  python -m node_diffusion_rid.precompute_room_ids "
                "--inp <原npz> --out <新npz>"
            )
        self.room_ids = d['room_ids'].astype(np.int32)   # [N, 40]
        print(f"NodeDataset(+room_ids): {len(self.coords)} samples from {npz_path}")
