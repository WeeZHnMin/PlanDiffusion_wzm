"""
Train NodeDiffusionTransformer+RoomID on preprocessed node-coordinate data.

使用方法（单卡）：
  python -m node_diffusion_rid.train \\
      --data_path data/processed/node_diffusion_rid/graph_dataset_5k.npz \\
      --save_dir  checkpoints/node_diffusion_rid/run1

多卡 DDP：
  torchrun --nproc_per_node=2 -m node_diffusion_rid.train \\
      --data_path data/processed/node_diffusion_rid/graph_dataset_5k.npz \\
      --save_dir  checkpoints/node_diffusion_rid/run1
"""
...
def build_parser(defaults=None):
    ...
    parser.add_argument("--data_path",    default=defaults.get("data_path", "data/processed/node_diffusion_rid/graph_dataset_5k.npz"))
    parser.add_argument("--save_dir",     default=defaults.get("save_dir",  "checkpoints/node_diffusion_rid"))
