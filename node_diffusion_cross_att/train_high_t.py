"""
在 train.py 基础上，将 t 的采样分布偏向高噪声区间，
以提升模型对不同初始噪声种子的鲁棒性。

改动：
    t = torch.randint(0, T, ...)        # 原来：均匀分布
    →  t ~ 线性加权，高 t 概率更大      # 现在：偏向高噪声

权重公式：
    weight[t] = 1 + t_bias * (t / (T-1))
    t_bias=0 → 等价于原始均匀采样
    t_bias=2 → t=999 的采样概率是 t=0 的 3 倍（默认）
    t_bias=9 → t=999 的采样概率是 t=0 的 10 倍

用法（单卡，从已有 checkpoint 继续训）：
    python -m node_diffusion_cross_att.train_high_t \\
        --resume checkpoints/node_diffusion_cross_att/latest.pt \\
        --save_dir checkpoints/node_diffusion_high_t \\
        --t_bias 2.0 \\
        --gpu 0

用法（torchrun 多卡）：
    torchrun --nproc_per_node=2 -m node_diffusion_cross_att.train_high_t \\
        --resume checkpoints/node_diffusion_cross_att/latest.pt \\
        --save_dir checkpoints/node_diffusion_high_t \\
        --t_bias 2.0
"""
...
def build_parser(defaults=None):
    ...
    parser.add_argument("--save_dir",    default=defaults.get("save_dir",  "checkpoints/node_diffusion_high_t"))
    parser.add_argument("--resume",      default="", help="从该 checkpoint 继续训练")
    ...
    parser.add_argument("--gpu",         type=int,   default=None,
                        help="单卡训练时指定 GPU 编号（多卡用 torchrun，不传此参数）")
    # ── 新增：t 采样偏置 ────────────────────────────────────────────────────────
    parser.add_argument("--t_bias",      type=float, default=2.0,
                        help="高 t 偏置强度。0=均匀（等同原始 train.py），"
                             "2=t=999 概率是 t=0 的 3 倍（默认），"
                             "9=t=999 概率是 t=0 的 10 倍")
    return parser


def make_t_weights(T: int, t_bias: float, device) -> torch.Tensor:
    """
    返回长度为 T 的权重向量，线性从 1 增长到 (1 + t_bias)。
    t_bias=0 → 全 1 → 等价于均匀采样。
    """
    w = 1.0 + t_bias * torch.arange(T, dtype=torch.float32) / max(T - 1, 1)
    return w.to(device)   # [T]


def sample_t(weights: torch.Tensor, B: int) -> torch.Tensor:
    """从预计算的权重分布中采样 B 个时间步。"""
    return torch.multinomial(weights.unsqueeze(0).expand(B, -1),
                             num_samples=1, replacement=True).squeeze(1)
...
def main(argv=None, defaults=None):
    ...
    if is_master:
        ...
        print(f"t_bias={args.t_bias}  "
              f"（t=999 采样概率是 t=0 的 {1 + args.t_bias:.1f} 倍）")
    ...
    t_weights  = make_t_weights(args.timesteps, args.t_bias, device)  # [T]
    ...
    for step in range(start_step, args.total_steps):
        ...
        # ── 唯一改动：偏向高 t 的采样分布 ────────────────────────────────────
        t = sample_t(t_weights, x.shape[0])
