...
class NodeDataset(Dataset):
    ...
    def __init__(self, npz_path):
        ...
        # BERT 重建的 NPZ 直接存有 attention_mask；旧版 BPE 按 prompt_lens 生成
        self._prompt_mask  = d['prompt_mask'].astype(np.int64) \
    ...
    def __getitem__(self, idx):
        ...
        if self._prompt_mask is not None:
            prompt_mask = self._prompt_mask[idx].astype(np.float32)
        else:
