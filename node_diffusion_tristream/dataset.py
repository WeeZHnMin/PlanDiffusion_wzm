...
class NodeDataset(Dataset):
    ...
    def __init__(self, npz_path):
        ...
        if 'room_membership' not in d:
            raise KeyError(
                f"npz '{npz_path}' 缺少 room_membership 字段。\n"
                "请重新运行 build_graph_npz.py 生成新的 npz。"
            )
        self.room_membership = d['room_membership'].astype(np.float32)  # [N, 40, MAX_ROOMS]
        print(f"NodeDataset(TriStream): {len(self.coords)} samples from {npz_path}")
