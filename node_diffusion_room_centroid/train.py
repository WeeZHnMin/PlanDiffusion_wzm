"""
Train NodeDiffusionTransformer（残差DDPM：z_0 = x_0 - x_centroid）。

使用方法（单卡）：
  python -m node_diffusion_room_centroid.train \\
      --data_path data/processed/node_diffusion_room_tri/graph_dataset.npz \\
      --save_dir  checkpoints/node_diffusion_room_centroid/run1

多卡 DDP：
  torchrun --nproc_per_node=2 -m node_diffusion_room_centroid.train \\
      --data_path data/processed/node_diffusion_room_tri/graph_dataset.npz \\
      --save_dir  checkpoints/node_diffusion_room_centroid/run1
"""
...
from .diffusion import GaussianDiffusion, compute_x_centroid
...
def _ddim_sample(model, diffusion, cond_batched, device, ddim_steps=200):
    """잔차 DDIM: 각 스텝마다 현재 x 로 x_centroid 재계산 후 z-공간에서 업데이트."""
    ...
    room_membership = cond_batched['room_membership']   # [B, N, R]

    # 순수 노이즈에서 시작 (절대 좌표 공간)
    ...
    for i, t in enumerate(ts):
        ...
        # 현재 x 추정치로 질심 계산 → 잔차 z
        x_cen = compute_x_centroid(x, room_membership)
        z     = x - x_cen
...
def _run_val(model, diffusion, tokenizer, val_records, args, device, step, log_file):
    ...
    for rec in sample:
        ...
        prepared.append({
            ...
            "gt_coords_pad": gt_coords_pad,
            "prompt": prompt,
        })

    micro_list, macro_list = [], []
    vis_samples = []   # (prepared_dict, pred_np) for first vis_n samples
    BS = args.val_batch
...
def _save_val_viz(vis_samples, step, vis_dir):
    """validation 시 첫 N 샘플을 5열 그리드로 저장."""
...
def build_parser(defaults=None):
    ...
    parser.add_argument("--save_dir",     default=defaults.get("save_dir",  "checkpoints/node_diffusion_room_centroid"))
    ...
    parser.add_argument("--total_steps",  type=int,   default=defaults.get("total_steps",  15000))
    ...
    parser.add_argument("--val_n",        type=int, default=defaults.get("val_n",        64))
    ...
    parser.add_argument("--vis_n",        type=int, default=defaults.get("vis_n",        5),
                        help="검증 시 시각화할 샘플 수")
    parser.add_argument("--vis_dir",      default=defaults.get("vis_dir", "outputs/node_diffusion_room_centroid"),
                        help="시각화 png 저장 디렉토리")
    return parser
