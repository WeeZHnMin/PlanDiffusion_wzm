...
def sample_coords(model, diffusion, room_mb_np: np.ndarray, adj_np: np.ndarray,
    ...
    """잔차 DDPM 1000步: 각 스텝마다 현재 x 로 x_centroid 재계산. 반환 [N_NODES, 2]."""
    ...
    for t in reversed(range(diffusion.T)):
        ...
        # 현재 x 에서 질심 계산 → 잔차로 모델 입력
        x_cen = compute_x_centroid(x, room_mb)
        z     = x - x_cen
        ...
        z0  = ((z - (1 - ab).sqrt() * eps) / ab.sqrt().clamp(min=1e-3)).clamp(-300, 300)
        x0  = z0 + x_cen   # 절대 좌표 복원

        a  = diffusion.alphas[t]
        b_ = diffusion.betas[t]
        # DDPM posterior mean in z-space, then convert back to x-space
        mu_z = (ap.sqrt() * b_ / (1 - ab)) * z0 + (a.sqrt() * (1 - ap) / (1 - ab)) * z
        x_cen_next = compute_x_centroid(x0, room_mb)
        if t > 0:
            z_next = mu_z + diffusion.posterior_variance[t].sqrt() * \
                     torch.randn(z.shape, device=device, generator=g)
            x = z_next + x_cen_next
        else:
            x = x0
...
def parse_args():
    ...
    p.add_argument('--ckpt',    default='checkpoints/node_diffusion_room_centroid/latest.pt')
    ...
    p.add_argument('--vocab',     default="node_diffusion_room_centroid/type_combo_vocab_v3.json")
    return p.parse_args()
