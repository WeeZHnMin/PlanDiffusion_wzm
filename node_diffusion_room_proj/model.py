"""
NodeDiffusionTransformer — 三流自注意力 + Per-Layer 文本投影

每个 EncoderLayer 并联三路自注意力，结果直接相加：
  - adj_attn   : 直接相邻节点（局部边约束，adj_matrix=1 的位置允许）
  - room_attn  : 同环所有节点（面约束，room_membership 推导）
  - global_attn: 全部有效节点（全局布局，仅屏蔽 padding）

文本条件改进：
  - BERT 输出由单一 Linear 投影改为每层独立 MLP（Linear→GELU→Linear）
  - 各层从同一 BERT 隐状态提取不同视角的文本特征，梯度互不叠加
  - BERT 最后 unfreeze_layers 层参与训练（默认2层）
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, room_mask, text_feat, text_mask, pad_mask=None, adj_mask=None):
        x2 = self.norm1(x)
        x  = x + self.dropout(
...
class NodeDiffusionTransformer(nn.Module):
    """
    三流自注意力：adj_attn + room_attn + global_attn。
    ...
    def __init__(self, model_channels=384, num_layers=6, num_heads=6,
        ...
        # 每个扩散层有自己独立的 MLP 投影：BERT hidden → d_model
        # Linear(768→384) → GELU → Linear(384→384)
        self.text_projs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(bert_hidden, model_channels),
                nn.GELU(),
                nn.Linear(model_channels, model_channels),
            )
        ...
        print(f"NodeDiffusionTransformer(tri-stream + per-layer proj, "
    ...
    def encode_text(self, prompt_tokens, prompt_mask=None):
        """
        返回原始 BERT 隐状态 [B, T, 768]，不做投影。
        per-layer 投影在 forward 的层循环里完成。
        BERT 最后 unfreeze_layers 层有梯度时不使用 no_grad。
        """
        ...
        return text_hidden, text_mask   # [B, T, 768], [B, 1, T]
    ...
    def forward(self, x, timesteps, node_mask,
        ...
        # bert_hidden: 原始 BERT 隐状态 [B, T, 768]
        # 优先使用预计算的（推理时避免每步重复跑 BERT）
        if text_feat is not None:
            bert_hidden = text_feat.to(dtype=dt)       # [B, T, 768]
            if text_mask is not None:
                text_mask = text_mask.to(dtype=dt)
        elif prompt_tokens is not None:
            bert_hidden, text_mask = self.encode_text(prompt_tokens, prompt_mask)
            bert_hidden = bert_hidden.to(dt)
            if text_mask is not None:
        ...
        for i, layer in enumerate(self.layers):
            if bert_hidden is not None:
                # 每层用自己的 MLP 提取不同视角的文本特征
                text_feat_i = self.text_projs[i](bert_hidden)   # [B, T, d_model]
            else:
                text_feat_i = torch.zeros(B, 1, self.model_channels,
                                          device=seq.device, dtype=dt)
            seq = layer(seq, room_mask, text_feat_i, text_mask, pad_mask, adj_mask)
