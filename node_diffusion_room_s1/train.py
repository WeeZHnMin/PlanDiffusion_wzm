...
def _draw_nodes(ax, coords_np, adj_np, mask_np, title):
    """coords_np: [MAX_NODES, 2]，画节点 + 边。"""
    ax.set_xticks([]); ax.set_yticks([])
    ax.set_facecolor('#F8F8F8')
    for sp in ax.spines.values():
        sp.set_edgecolor('#CCCCCC'); sp.set_linewidth(0.4)
        ...
        for jj in range(ii + 1, len(valid)):
            ...
            if adj_np[ni, nj] > 0.5:
                ax.plot([d[ii,0], d[jj,0]], [d[ii,1], d[jj,1]],
                        color='#AAAAAA', lw=0.7, alpha=0.7, zorder=1)
    ax.scatter(d[:,0], d[:,1], s=22, c='#4E8CC2',
               edgecolors='#334455', linewidths=0.4, zorder=3)
...
def _run_val(model, diffusion, tokenizer, val_records, args, device, step, log_file, save_dir):
    ...
    for rec in sample:
        ...
        prepared.append({
            ...
            "combo_ids": combo_ids, "gt_coords": gt_coords,
            "ptok": ptok, "pmsk": pmsk, "n": n,
        })

    # ── DDIM 采样 + 质心 RMSE ────────────────────────────────────────────────
    rmse_list  = []
    pred_store = []   # 存前几条用于可视化
    BS = args.val_batch
    ...
    for bi in range(0, len(prepared), BS):
        ...
        for j, s in enumerate(chunk):
            ...
            memb    = s["membership"]                       # [MAX_NODES, MAX_ROOMS]
            gt_c    = s["gt_coords"]                        # [MAX_NODES, 2]
            valid   = np.where(s["mask_np"] > 0.5)[0]

            ring_errs = []
            for r in range(memb.shape[1]):
                nodes_r = [i for i in valid if memb[i, r] > 0.5]
                if len(nodes_r) == 0:
                    continue
                pred_cent = pred_np[nodes_r].mean(0)
                gt_cent   = gt_c[nodes_r].mean(0)
                ring_errs.append(float(np.sum((pred_cent - gt_cent) ** 2)))
            if ring_errs:
                rmse_list.append(float(np.sqrt(np.mean(ring_errs))))

            if len(pred_store) < 3:
                pred_store.append((s, pred_np))

    centroid_rmse = float(np.mean(rmse_list)) if rmse_list else 9999.0
    elapsed = time.perf_counter() - t0
    print(f"[val step {step:6d}] n={len(rmse_list)} | "
          f"centroid_rmse={centroid_rmse:.2f} px | {elapsed:.1f}s")
    log_file.write(json.dumps({
        'step': step,
        'centroid_rmse': round(centroid_rmse, 3),
        'elapsed_val': round(elapsed, 1),
    }) + '\n')

    # ── 可视化：GT正向扩散(t=T,2T/3,T/3,0) + Pred ───────────────────────────
    snap_ts_code = [diffusion.T - 1,
                    (diffusion.T * 2) // 3,
                    diffusion.T // 3,
                    0]
    snap_labels  = [f't={diffusion.T}', f't={diffusion.T*2//3}',
                    f't={diffusion.T//3}', 't=0 (GT)']

    n_vis  = len(pred_store)
    n_cols = len(snap_ts_code) + 1   # GT snapshots + pred
    if n_vis > 0:
        fig, axes = plt.subplots(n_vis, n_cols,
                                 figsize=(2.6 * n_cols, 2.6 * n_vis),
                                 constrained_layout=True)
        if n_vis == 1:
            axes = [axes]

        for row, (s, pred_np) in enumerate(pred_store):
            gt_x0 = torch.from_numpy(s["gt_coords"].T[None]).float().to(device)  # [1,2,N]

            for col, (t_code, label) in enumerate(zip(snap_ts_code, snap_labels)):
                ax = axes[row][col]
                if t_code == 0:
                    noisy_np = s["gt_coords"]
                else:
                    t_idx = torch.tensor([t_code], dtype=torch.long, device=device)
                    xt, _ = diffusion.q_sample(gt_x0, t_idx)
                    noisy_np = xt.squeeze(0).permute(1, 0).cpu().numpy()  # [N,2]
                _draw_nodes(ax, noisy_np, s["adj_pad"], s["mask_np"], label)

            # pred 列
            ax_pred = axes[row][n_cols - 1]
            _draw_nodes(ax_pred, pred_np, s["adj_pad"], s["mask_np"], 'Pred')

        vis_path = Path(save_dir) / f"val_step{step:06d}.png"
        fig.savefig(vis_path, dpi=140, bbox_inches='tight')
        plt.close(fig)
        print(f"  vis -> {vis_path}")

    raw_model.train()
    return centroid_rmse
...
def main(argv=None, defaults=None):
    ...
    for step in range(start_step, args.total_steps):
        ...
        if is_master and val_records and step > 0 and step % args.val_interval == 0:
            centroid_rmse = _run_val(model, diffusion, val_tokenizer, val_records,
                                     args, device, step, log_file, save_dir)
            if centroid_rmse < best_centroid_rmse:
                best_centroid_rmse = centroid_rmse
                raw_model = model.module if use_ddp else model
                ...
                print(f"  best model saved (centroid_rmse={centroid_rmse:.2f}) -> {best_path}")
