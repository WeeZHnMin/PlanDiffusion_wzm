...
Dataset figure: 3 rows × 3 columns
  Col 0: floor plan image (from data/viz_50000/)
...
# ── Figure layout ────────────────────────────────────────────────────────────
N_ROWS  = len(records)
ROW_H   = 2.2      # inches per row
FIG_W   = 7.2
LEGEND_H = 0.42    # inches reserved at bottom for shared legend
...
row_h_frac = (1 - T - B - GAP_ROW * (N_ROWS - 1)) / N_ROWS

def make_ax(row, col):
    y0  = B + (N_ROWS - 1 - row) * (row_h_frac + GAP_ROW)
    x0  = L + sum(COL_RATIOS[:col]) / total_r * (1 - L - R) + GAP_COL * col
    w   = COL_RATIOS[col] / total_r * (1 - L - R) - GAP_COL
    return fig.add_axes([x0, y0, w, row_h_frac])
...
for row, rec in enumerate(records):
    ...
    # ── Col 0: floor plan image ──────────────────────────────────────────────
    ax_img = axes[row][0]
    try:
        img = Image.open(f"{VIZ_DIR}/{rec['image']}")
        ax_img.imshow(img)
    ...
    ax_img.axis('off')
    if row == 0:
        ax_img.set_title('Floor Plan', fontsize=8.5, fontweight='bold', pad=4)
    ...
    if row == 0:
        ax_g.set_title('Vertex Graph', fontsize=8.5, fontweight='bold', pad=4)
    ...
    if row == 0:
        ax_t.set_title('Text Description', fontsize=8.5, fontweight='bold', pad=4)
...
ax_leg.legend(handles=legend_handles,
              loc='center', ncol=6,
              fontsize=6.0, frameon=True,
              framealpha=0.9, edgecolor='#CCCCCC',
              columnspacing=0.7, handlelength=1.0, handletextpad=0.4,
              borderpad=0.5, title='Node Type', title_fontsize=6.5)
