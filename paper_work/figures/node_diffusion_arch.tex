...
\node[B={Ci}] (adj) at ( 4.8,  0)  {Adjacency \&\\[-2pt]Room Membership};
...
\node[HW={Cs}] (ga) at (-1.65, -6.85) {Global\\[-2pt]\footnotesize Self-Attn};
\node[HW={Cs}] (aa) at ( 1.65, -6.85) {Adj/Room\\[-2pt]\footnotesize Self-Attn};
...
% ln1 → two independent vertical arrows to adj and global
\draw[a] (ln1.south -| aa.north) -- (aa.north);
\draw[a] (ln1.south -| ga.north) -- (ga.north);

% adj and global outputs → add1  (down → horizontal → down)
