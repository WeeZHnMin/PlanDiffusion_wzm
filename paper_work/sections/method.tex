...
组合类型 ID（$40$）及文本描述（WordPiece 编码，长度不超过 128 token）。
...
坐标 $\mathbf{x}_0 \in \mathbb{R}^{N \times 2}$ 可直接在欧氏空间扩散。
...
两路前向过程如图~\ref{fig:diffusion} 所示，分别为：
\begin{align}
...
去噪网络 $f_\theta$ 为双流注意力 Transformer（架构详见图~\ref{fig:arch}），
...
\begin{equation}
    ...
    \label{eq:model}
\end{equation}
其中 $\mathbf{E}_t \in \mathbb{R}^{N \times d}$ 为加噪类型嵌入矩阵，第 $i$ 行为 $\mathbf{e}_t^{(i)}$。
...
从 $(\mathbf{x}_T, \mathbf{E}_T) \sim \mathcal{N}(\mathbf{0},\mathbf{I})$
...
\FOR{$k = 1,\, 2,\, \ldots,\, N-1$}
    \STATE 屏蔽编号 $\geq k$ 的节点 token 及 \texttt{SEP} \COMMENT{约束①：因果树约束 $p_k < k$；约束②：SEP 时机}
    \STATE 采样 $p_k \sim p_{\theta_1}(\cdot \mid s_{<\mathrm{cur}})$
\ENDFOR
\STATE 强制插入 \texttt{SEP} \COMMENT{约束②：恰好 $N-1$ 个父节点后强制 SEP}
\WHILE{未采样到 \texttt{EOS\_G}}
    \STATE 屏蔽所有会使当前图出现三角环的节点 token \COMMENT{约束③：禁止三角环}
    \IF{$\exists\, i : \deg(v_i) < 2$}
        \STATE 屏蔽 \texttt{EOS\_G} \COMMENT{约束④：度数下限，所有节点度 $\geq 2$ 方可结束}
    \ENDIF
