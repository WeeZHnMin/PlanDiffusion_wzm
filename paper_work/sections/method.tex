...
与之对比，NodeDiffusionTransformer 为 $d=256$，6 层，4 头。
...
（2）坐标扩散：以 $(\mathbf{A}, \mathbf{m}, \mathcal{T})$ 为条件，
从 $\mathbf{x}_T \sim \mathcal{N}(\mathbf{0},\mathbf{I})$
出发执行 1000 步逆扩散，得到节点坐标 $\hat{\mathbf{X}}$；
（3）类型预测：以 $(\hat{\mathbf{X}}, \mathbf{A}, \mathcal{T})$ 为条件，
NodeTypeClassifier 输出每节点类型 $\hat{\mathbf{y}}$；
最终反归一化坐标，结合 $\mathbf{A}$ 与 $\hat{\mathbf{y}}$ 渲染矢量平面图。
...
\STATE $\hat{y}_i = \arg\max_{k} \hat{z}_{i,k}$，$\forall i$
