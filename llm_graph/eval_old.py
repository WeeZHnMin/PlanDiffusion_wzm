...
from .infer_stage1 import (
    generate, load_model, parse_sequence, node_degrees,
    encode_text, BOS_ID,
)
...
def evaluate(model, rows, vocab, device, temperature=1.0):
    ged_list        = []
    face_diff_list  = []
    ...
    for i, rec in enumerate(rows):
        ...
        gt = {'n_nodes': n, 'adj': gt_adj, 'valid': True}
        ...
        if gen['valid']:
            ...
            face_diff_list.append(abs(face_count(gen['adj']) - face_count(gt['adj'])))
...
def parse_args():
    ...
    p.add_argument('--n-samples',   type=int,   default=0, help='评估样本数（0=全部）')
    p.add_argument('--temperature', type=float, default=1.0)
    p.add_argument('--seed',        type=int,   default=42)
    p.add_argument('--out',         default='outputs/eval_old.json')
    return p.parse_args()
...
def main():
    ...
    print(f'评估样本数: {len(rows)}')

    results = evaluate(model, rows, args.vocab, device, temperature=args.temperature)
