"""
Stage1：纯图序列自回归预训练（不含文本条件）。
序列：BOS_G [N_tok] [parents] SEP [extra_edges] EOS_G

用法：
  python -m llm_graph.train_stage1
  python -m llm_graph.train_stage1 --resume checkpoints/llm_graph/stage1/best.pt
"""
...
def parse_args():
    ...
    p.add_argument("--max-pos-emb",       type=int, default=256)
...
def main():
    ...
    loader = make_loader(args.data, args.batch_size, stage=1,
    ...
    for step in range(start_step, total_steps):
        ...
        running['acc_N']      += m['acc_N'] if not (m['acc_N'] != m['acc_N']) else 0
        running['acc_parent'] += m['acc_parent'] if not (m['acc_parent'] != m['acc_parent']) else 0
        running['acc_edge']   += m['acc_edge'] if not (m['acc_edge'] != m['acc_edge']) else 0
        save_win_loss  += lv
        save_win_steps += 1
