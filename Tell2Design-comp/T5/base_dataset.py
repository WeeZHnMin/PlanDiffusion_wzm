...
def seq2seq_data_collator(features):
    ...
    batch = {}
    fixed_keys = ["input_ids", "attention_mask", "label_ids", "boundary_ids", "boundary_mask", "decoder_attention_mask"]
    for key in fixed_keys:
        if key not in normalized[0] or normalized[0][key] is None:
            continue
        batch_key = "labels" if key == "label_ids" else key
        batch[batch_key] = torch.tensor([feat[key] for feat in normalized], dtype=torch.long)
    return batch
