...
class GaussianDiffusion:
    ...
    轮廓节点（is_boundary=1）在整个扩散过程中坐标固定：
      - q_sample: 轮廓节点保持 x0，不加噪声
      - training_losses: 损失只计算非轮廓有效节点
    """
    ...
    def q_sample(self, x0, t, noise=None, is_boundary=None):
        """
        前向扩散。轮廓节点（is_boundary=1）不加噪声，保持真实坐标 x0。
        is_boundary: [B, N] or None
        """
        if noise is None:
        ...
        if is_boundary is not None:
            bnd = is_boundary.unsqueeze(1).bool()   # [B, 1, N]
            x_t = torch.where(bnd, x0.float(), x_t)
        return x_t, noise
    ...
    def training_losses(self, model, x0, t, model_kwargs):
        ...
        is_boundary = model_kwargs.get('is_boundary', None)
        coord_noise = torch.randn_like(x0)
        xt, _       = self.q_sample(x0, t, coord_noise, is_boundary)
        ...
        if is_boundary is not None:
            # 损失只计算非轮廓有效节点
            non_bnd    = (1 - is_boundary.float())
            coord_mask = (node_mask * non_bnd).unsqueeze(1)   # [B, 1, N]
        else:
