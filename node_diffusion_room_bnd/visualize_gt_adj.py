"""
用测试集 GT 邻接矩阵 + 文本条件 + 建筑轮廓条件，运行 θ₂→render 并可视化。

只测试 node_diffusion_room_bnd（坐标扩散模型），不需要 θ₃ 类型分类器。
节点类型直接从 JSONL 的 node_types 字段读取，用于渲染着色。
轮廓节点（is_boundary=1）由半边算法从 GT 邻接矩阵实时计算，
推理时每步强制将其坐标固定为 GT 真实值。

输出 N 行 × 4 列图：
  Col1  输入文本描述
  Col2  GT 邻接图（spring layout，轮廓节点橙色）
  Col3  θ₂ 预测坐标图
  Col4  渲染平面图（GT 节点类型着色）

用法（项目根目录）：
    python -m node_diffusion_room_bnd.visualize_gt_adj \\
        --ckpt   checkpoints/node_diffusion_room_bnd/run1/latest.pt \\
        --data   data/jsonl/final_graph_dataset_v3.jsonl \\
        --n      5 \\
        --out    outputs/visualize_gt_adj_room_bnd/result.png
"""
...
def _find_outer_face_nodes(coords_list, adj_list, n):
    ...
    outer_set = set(all_faces[int(np.argmax(abs_areas))])
...
def draw_col2_adj(ax, adj_np: np.ndarray, n: int, is_bnd_np: np.ndarray = None, seed: int = 0):
    _ax_style(ax)
    if n == 0:
        return
    ...
    for i in range(n):
        is_bnd = is_bnd_np is not None and is_bnd_np[i] > 0.5
        color  = '#E87722' if is_bnd else '#4E8CC2'   # 橙色=轮廓，蓝色=内部
        ax.add_patch(plt.Circle((pos[i, 0], pos[i, 1]), 0.11,
                                color=color, ec='#333333', lw=0.6, zorder=3))
...
@torch.no_grad()
def sample_coords(model, diffusion,
                  room_mb_np: np.ndarray, adj_np: np.ndarray,
                  mask_np: np.ndarray, ptok_np: np.ndarray, pmsk_np: np.ndarray,
                  is_bnd_np: np.ndarray, gt_coords_np: np.ndarray,
                  device, seed: int = 0) -> np.ndarray:
    """
    DDPM 1000步逆采样，四流条件：room_membership + adj_matrix + text + is_boundary。

    轮廓节点在每步采样后强制覆盖为 GT 真实坐标（is_bnd_np=1 的位置）。
    返回 [N_NODES, 2]。
    """
    ...
    is_bnd   = torch.from_numpy(is_bnd_np[None]).float().to(device)       # [1, N]
...
def parse_args():
    ...
    p.add_argument('--out',     default='outputs/visualize_gt_adj_room_bnd/result.png')
    return p.parse_args()
...
def main():
    ...
    for idx in indices:
        ...
        # is_boundary：用半边算法从 GT 坐标 + 邻接矩阵计算
        coords_ll = [(float(gt_coords_np[k, 0]), float(gt_coords_np[k, 1]))
                     for k in range(n_eff)]
        adj_ll    = [[int(adj_np[i, j]) for j in range(n_eff)] for i in range(n_eff)]
        bnd_n     = _find_outer_face_nodes(coords_ll, adj_ll, n_eff)
        is_bnd_np = np.zeros(N_NODES, dtype=np.float32)
        is_bnd_np[:n_eff] = bnd_n.astype(np.float32)
        ...
        n_bnd = int(bnd_n.sum())
        print(f'  [idx={idx}] n={n_eff}  bnd={n_bnd}  text={text[:60]}...')
        records.append(dict(
    ...
    # ── DDPM 采样 ─────────────────────────────────────────────────────────────
    for rec in records:
        print(f'  DDPM 1000步  idx={rec["idx"]}  bnd={int(rec["is_bnd_np"].sum())}...', flush=True)
        rec['pred_coords'] = sample_coords(
            model, diffusion,
            rec['room_mb_np'], rec['adj_np'],
            rec['mask_np'], rec['ptok_np'], rec['pmsk_np'],
            rec['is_bnd_np'], rec['gt_coords_np'],
            device, seed=rec['idx'],
        )   # [40, 2]
    ...
    for row_i, rec in enumerate(records):
        ...
        draw_col2_adj    (axs[1], rec['adj_np'], rec['n_nodes'],
                          is_bnd_np=rec['is_bnd_np'], seed=args.seed)
