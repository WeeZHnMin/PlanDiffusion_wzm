"""
NodeDataset for node_diffusion_room_bnd（四流注意力 + 建筑轮廓条件）：
cond 中附带 room_membership [N, MAX_ROOMS]、adj_matrix [N, N]、
is_boundary [N]（由 build_graph_npz.py 预计算并存入 npz）。

npz 中必须含有 room_membership、adj_matrix、is_boundary 字段。
"""
...
class NodeDataset(Dataset):
    ...
    Each sample returns:
      ...
      cond : dict with
               ...
               is_boundary     [40]              1=轮廓节点（坐标固定），0=内部节点
    """
    ...
    def __init__(self, npz_path):
        ...
        for field in ('room_membership', 'adj_matrix', 'is_boundary'):
            if field not in d:
                raise KeyError(
                    f"npz '{npz_path}' 缺少 {field} 字段。\n"
                    "请重新运行 node_diffusion_room_bnd.build_graph_npz 生成新 npz。"
                )
        ...
        self.is_boundary     = d['is_boundary'].astype(np.float32)      # [N, 40]
        print(f"NodeDataset(BndStream): {len(self.coords)} samples from {npz_path}")
    ...
    def __getitem__(self, idx):
        ...
        cond = {
            ...
            'is_boundary':     self.is_boundary[idx],   # [40]
        }
